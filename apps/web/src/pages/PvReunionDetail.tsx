...
export default function PvReunionDetail() {
  ...
  const handleDelete = async () => {
    if (!id || !confirm('Mettre ce PV à la corbeille ?')) return;
    try {
  ...
  if (loading) {
    return (
      <div className="max-w-5xl mx-auto p-8 text-center text-gray-500">Chargement…</div>
    );
  }
  ...
  if (!pv) {
    return (
      <div className="max-w-5xl mx-auto p-8">
        ...
        <Link to="/pv-reunion" className="text-blue-600 hover:underline mt-4 inline-block">
          ← Retour à la liste
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <div className="mb-6">
        <Link to="/pv-reunion" className="text-sm text-blue-600 hover:underline">
          ← Liste des PV
        </Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow border border-gray-100 p-6 mb-6">
        <div className="flex flex-wrap justify-between gap-4 mb-4">
          <h1 className="text-2xl font-bold text-gray-900">{pv.titre}</h1>
            ...
            {canEdit && !editMode && (
              <>
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-sm border border-red-200 text-red-700 rounded-lg"
                >
                  Supprimer
                </button>
              </>
      ...
      {!editMode && (
        <>
          <section className="bg-white rounded-xl shadow border border-gray-100 p-6 mb-6">
              ...
              {pv.presentsClientFournisseur?.length
                ? pv.presentsClientFournisseur.map((x: any) => (
                    <span key={x.id || x.clientFournisseurId} className="px-2 py-1 bg-amber-50 rounded">
                      {x.clientFournisseur?.raisonSociale || x.clientFournisseurId}
                    </span>
                  ))
                : null}
